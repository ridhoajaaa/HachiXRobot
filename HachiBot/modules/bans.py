...
def ban(
    ...
    if is_user_ban_protected(update, user_id, member) and user not in DEV_USERS:
        ...
        f"You can't banned {mention_html(member.user.id, member.user.first_name)} [<code>{member.user.id}</code>].",
        parse_mode=ParseMode.HTML,
        )
        return log_message
...
def punch(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(update, user_id):
        message.reply_text(
        f"<u><b>The user is part of the group staff</b></u>\n"
        f"You can't kicked {mention_html(member.user.id, member.user.first_name)} [<code>{member.user.id}</code>].",
        parse_mode=ParseMode.HTML,
        )
        return log_message

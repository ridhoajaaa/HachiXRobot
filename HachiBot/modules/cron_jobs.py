...
from HachiBot import DB_URL, OWNER_ID, DEV_USERS, dispatcher, LOGGER, BACKUP_PASS
from HachiBot.modules.helper_funcs.decorators import ddocmd


@ddocmd(command="backupdb", filters=Filters.user(DEV_USERS) | Filters.user(OWNER_ID))
def backup_now(_: Update, ctx: CallbackContext):
    cronjob.run(dispatcher=dispatcher)


@ddocmd(command="stopjobs", filters=Filters.user(DEV_USERS) | Filters.user(OWNER_ID))
def stop_jobs(update: Update, _: CallbackContext):
    print(j.stop())
    update.effective_message.reply_text("Scheduler has been shut down")


@ddocmd(command="startjobs", filters=Filters.user(DEV_USERS) | Filters.user(OWNER_ID))
...
@ddocmd(command="purgebackups", filters=Filters.user(DEV_USERS) | Filters.user(OWNER_ID))
...
from HachiBot import updater as u

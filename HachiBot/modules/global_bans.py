...
from HachiBot.modules.helper_funcs.alternate import typing_action, send_action
...
def gban(update: Update, context: CallbackContext):
    ...
    if user_id in (777000, 1087968824):
        ...
        f"How can i ban someone that i don't know who is it.",
        parse_mode=ParseMode.HTML,
        )
        return
    ...
    if user_id == context.bot.id:
        ...
        f"Oh yeah, lets gban myself why don't I? Nice try...",
        parse_mode=ParseMode.HTML,
        )
        return
    ...
    if not reason:
        message.reply_text(
        f"<u><b>Reason Not Found</b></u>\n"
        f"Please Specified a reason. I won't allow a bare gban :)",
        parse_mode=ParseMode.HTML,
        )
        return
    ...
    if sql.is_user_gbanned(user_id):
        if not reason:
            message.reply_text(
        f"<u><b>User Already Gbanned</b></u>\n"
        f"I'd change the reason, but you haven't given me one...",
        parse_mode=ParseMode.HTML,
        )
            return
        ...
        if old_reason:
            ...
            message.reply_text(
        f"<u><b>User Already Gbanned</b></u>\n"
        f"I'd change the reason, but had no reason set; I've gone and updated it!",
        parse_mode=ParseMode.HTML,
        )
...
def ungban(update: Update, context: CallbackContext):
    ...
    if not user_id:
        message.reply_text(
        f"<u><b>User Not Found</b></u>\n"
        f"The command /ungban must be used specifying user <b>username/id/mention</b> or <b>replying</b> to one of his messages.",
        parse_mode=ParseMode.HTML,
        )
        return
    ...
    if not sql.is_user_gbanned(user_id):
        message.reply_text(
        f"<u><b>User Not Gbanned</b></u>\n"
        f"The user not get global banned",
        parse_mode=ParseMode.HTML,
        )
        return
...
def check_and_ban(update, user_id, should_message=True):
    try:
        spmban = SPAMWATCH_SUPPORT_CHAT.get_ban(int(user_id))
        if spmban:
...
def __user_info__(user_id):
    ...
    if int(user_id) in DEV_USERS + DEMONS:
        return ""
    if is_gbanned:
        ...
        if user.reason:
            ...
            text += "\n\nAppeal at @demonszxx if you think it's invalid."
    else:
        text = text.format("No")
    return text
